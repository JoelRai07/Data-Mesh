...
Unveränderte Zeilen werden gar nicht mehr angefasst. `ensure_iceberg_audit_table()` legt eine fehlende Tabelle frisch als Iceberg an; existiert ein Altbestand noch als Parquet, migriert `_migrate_parquet_to_iceberg()` ihn automatisch verlustfrei (CTAS + Zeilenzahl-Check + `RENAME`-Swap, Original bleibt bis zum verifizierten Tausch unangetastet) — direkt in der Pipeline, kein separates Skript mehr nötig.
...
**Trade-offs:** `MERGE`/`DELETE` sind jüngere Impala-Features (aber gegen den zuvor selbstgebauten Workaround kein Rückschritt); die Parquet→Iceberg-Migration eines Altbestands läuft automatisch in der Pipeline (verlustfrei, Original bis zum verifizierten Swap unangetastet), betrifft aber Produktionsdaten und wird daher beim ersten Lauf protokolliert; **kein Compaction/Snapshot-Management** ergänzt (Iceberg läuft `merge-on-read` → über sehr viele Läufe sammeln sich Delete-Dateien; bei dieser Datenmenge unkritisch, bewusst als Ausblick offen gelassen).
...
Problemliste (P1–P9 mit Fallstudien, nicht behebbare Einschränkungen, offene Punkte).
