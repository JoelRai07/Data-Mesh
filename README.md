...
- **Zeilengenauer Merge** (Bauland, Bevölkerung — Tabellen mit Business-Key):
  FNV-Hash je Zeile, nur neue/geänderte/gelöschte Keys werden ersetzt. Die
  beiden Audit-Tabellen laufen dafür als **Apache-Iceberg-Tabellen** — echtes
  `DELETE` + `MERGE INTO` statt des früheren `CREATE TABLE … AS SELECT` +
  Rename-Swap (Parquet in Impala kennt kein UPDATE/MERGE). Begründung,
  Verifikation und Historie: [ADR-13](docs/ADR.md).
...
jeweiligen Skripte und in [ADR.md](docs/ADR.md).

## Projektstruktur

```
Data-Mesh/
├── README.md                        # Diese Datei (Projekt, Architektur, Datenmodell, Setup, Benutzung)
...
**In `gruppe3` erzeugt die Pipeline vier Schichten** (Zeilenzahlen live
verifiziert am 09.07.2026):

| Schicht | Tabellen | Zeilen | Zweck |
|---|---|---|---|
| Staging | `gruppe3_staging_{gemeinden, bauland, klimadaten, bevoelkerungzahlen}` | 10.950 / 21.600 / 8.599.212 / 581 | unveränderte Rohkopie |
| Audit | `gruppe3_audit_{gemeinden, bauland, klimadaten, bevoelkerungzahlen}` | 10.950 / 21.600 / 262.359 / 581 | bereinigte Basis (`bauland`+`bevoelkerungzahlen` als Iceberg, Rest Parquet; Klima gefiltert auf Germany) |
| Datenprodukt | 4 × `gruppe3_dim_*`, 5 × `gruppe3_fact_*` | s. unten | Star-Schema für Konsumenten |
...
- [x] **Kompletter `run_pipeline.py`-Lauf grün** (09.07.2026): alle Quellen
  unverändert erkannt → Stufe 1+2 übersprungen, Spark-Lauf übersprungen,
  **Data-Contract-Gate: 32 Checks OK, 0 Fehler** — das Skip-Verhalten des
  Incremental Loadings ist damit End-to-End demonstriert.
- [x] 0 kaputte Zeichen in allen Audit-Tabellen (live geprüft 09.07.2026)
- [ ] Offene Restpunkte (Präsentation, Kür-Ideen): [Probleme.md → Offene Punkte](docs/Probleme.md)
